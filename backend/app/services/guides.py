...
def apply_larp_columns(guide: Guide, document: GuideDocument) -> None:
    """Copy the verdict layer onto the guide row so cards and filters stay cheap."""
    profile = document.content.larp
    guide.entry_type = profile.entry_type
    guide.verdict = profile.verdict
    guide.exposure_seconds = profile.exposure_seconds
...
def larp_card(guide: Guide) -> LarpCard:
    return LarpCard(
        ...
        exposure_seconds=guide.exposure_seconds,
        unfalsifiable=guide.unfalsifiable,
        flags=list(guide.flags or []),
        dek=guide.dek,
    )
...
def create_guide(db: Session, document: GuideDocument, author: User) -> Guide:
    ...
    apply_larp_columns(guide, document)
    db.add(guide)
    db.flush()
